...
- Logical addresses are virtual references generated by the CPU during program execution, representing the memory view from the process's perspective.
- Physical addresses are the actual, fixed locations in the hardware memory (RAM) where data is stored

## Requirements
- The RAM is simulated by an array of 2^7 (128) bytes. 
- The page size (also frame size) are 2^4 (16) bytes. 
...
- The logical address is then translated to a physical address and the data is read from the RAM.
- The second access to addresses in the same page should not cause a page fault unless that page has been removed.
- The sequences of 300 addresses that CPU needs is available in addresses.txt. 
